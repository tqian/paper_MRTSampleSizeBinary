...
for (i in 1:nrow(SD)) {
    ...
    ## taut_w
    taut_w <- construct_taut_theta(m, SD$AvgTau_w[i], SD$tau_w_shape[i], SD$tau_w_theta[i])
...
if (0) {
    ...
    result %>% 
        ...
        facet_grid(ft_t_shape + ft_t_theta ~ gt_t_shape + gt_t_theta,
                   labeller = label_both) +
        theme_bw() + myggfont()
    ggsave(paste0("plot_simu9.1_power.pdf"), width = 10, height = 7.5)
    ...
    result %>% 
        ...
        facet_grid(ft_t_shape + ft_t_theta ~ gt_t_shape + gt_t_theta,
                   labeller = label_both) +
        theme_bw() + myggfont()
    ggsave(paste0("plot_simu9.1_typeierror.pdf"), width = 10, height = 7.5)
}