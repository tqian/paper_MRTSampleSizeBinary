...
#
# Working assumption (a) violated:
# f_t and f_w are of the same form, but ATE_w > or < than ATE_t
#
# Tianchen Qian
# 2022.05.07
...
SD$ATE_w <- SD$ATE_t / SD$ATE_tw_ratio
...
if (0) {
    ...
    result %>% 
        ...
        theme_bw() + myggfont()
    ggsave(paste0("plot_simu4_power.pdf"), width = 10, height = 7.5)
    ...
    result %>% 
        ...
        facet_grid(ft_t_shape ~ ft_t_theta, labeller = label_both) +
        theme_bw() + myggfont()
    ggsave(paste0("plot_simu4_typeierror.pdf"), width = 10, height = 7.5)
}
