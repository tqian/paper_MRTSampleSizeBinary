...
if (0) {
    ...
    result %>% 
        ...
        ggplot(aes(x = gt_w_theta, y = power, linetype = ss_correction,
                   color = ATE_ASPNC,
        ...
        facet_grid(ft_t_shape + ft_w_shape ~ gt_t_shape + gt_w_shape + ft_t_theta,
                   labeller = label_both) +
        theme_bw() + myggfont()
    ggsave(paste0("plot_simu6.5_power.pdf"), width = 10, height = 7.5)
    
    result %>% 
        mutate(ATE_ASPNC = paste0(ATE_t, ", ", ASPNC_t)) %>%
        pivot_longer(c("typeierror_adj", "typeierror_unadj"),
                     names_to = "ss_correction", values_to = "typeierror") %>%
        ggplot(aes(x = gt_w_theta, y = typeierror, linetype = ss_correction,
                   color = ATE_ASPNC,
                   group = interaction(ATE_t, ASPNC_t, ss_correction))) +
        # geom_point() +
        geom_line() +
        ...
        facet_grid(ft_t_shape + ft_w_shape ~ gt_t_shape + gt_w_shape + ft_t_theta,
                   labeller = label_both) +
        theme_bw() + myggfont()
    ggsave(paste0("plot_simu6.5_typeierror.pdf"), width = 10, height = 7.5)
}
