...
SD <- expand.grid(gamma = 0.05,  # type I error
                  ...
                  coef_DE = seq(from = - 0.5, to = 0.5, by = 0.05),
...
for (i in 1:nrow(SD)) {
    ...
    SD$q[i] <- ncol(gt_t)
    alpha_t <- solve_alpha_DEonY(SD$gt_t_shape[i], m, SD$ASPNC_t[i], gt_t, taut_t, 
                                 coef_DE, pt, theta = SD$gt_t_theta[i])
    SD[i, c("alpha_t0", "alpha_t1", "alpha_t2")[1:SD$q[i]]] <- alpha_t
    SD$ASPNC_t_empirical[i] <- compute_ASPNC_DEonY(gt_t, alpha_t, taut_t, coef_DE, pt)
    stopifnot(all.equal(SD$ASPNC_t[i], SD$ASPNC_t_empirical[i]))
    
    ASPNC_if_no_DE <- compute_ASPNC(gt_t, alpha_t, taut_t)
    
    ## ft_t, beta_t
    ft_t <- construct_ftgt(m, SD$ft_t_shape[i])
    SD$p[i] <- ncol(ft_t)
    beta_t <- solve_beta_DEonY(SD$ft_t_shape[i], m, SD$ATE_t[i], ft_t, gt_t, alpha_t, 
                         taut_t, theta = SD$ft_t_theta[i])
    SD[i, c("beta_t0", "beta_t1", "beta_t2")[1:SD$p[i]]] <- beta_t
    SD$ATE_t_empirical[i] <- compute_ATE_DEonY(gt_t, alpha_t, ft_t, beta_t, taut_t)
    ...
    t <- try(dgm_DEonY(SD$n[i], m, ft_t, beta_t, gt_t, alpha_t, taut_t, pt, coef_DE))
...
for (i in 137:nrow(SD)) {
    ...
    for (truth_hypothesis in c("H1", "H0")) {
        ...
        result <- foreach(isim = 1:nsim, .combine = "c", .options.snow = opts) %dorng% {
            library(rootSolve)
            dta <- dgm_DEonY(n, m, ft_t, beta_t, gt_t, alpha_t, taut_t, pt, coef_DE)
...
if (0) {
    ...
    result %>% 
        ...
        facet_grid(ft_t_theta ~ gt_t_theta,
                   labeller = label_both) +
        theme_bw() + myggfont()
    ggsave(paste0("plot_simu7.1_power.pdf"), width = 10, height = 7.5)
    ...
    result %>% 
        ...
        facet_grid(ft_t_theta ~ gt_t_theta,
                   labeller = label_both) +
        theme_bw() + myggfont()
    ggsave(paste0("plot_simu7.1_typeierror.pdf"), width = 10, height = 7.5)
}

if (0) {
    tmp1 <- readRDS("result_df_collected_nsim2000_1-136.RDS")
    tmp2 <- readRDS("result_df_collected_nsim2000_137+.RDS")
    tmp <- rbind(tmp1, tmp2)
    saveRDS(tmp, file = "result_df_collected_nsim2000.RDS")
    
    tmp1 <- readRDS("result_list_collected_nsim2000_1-136.RDS")
    tmp2 <- readRDS("result_list_collected_nsim2000_137+.RDS")
    tmp <- c(tmp1, tmp2)
    saveRDS(tmp, file = "result_list_collected_nsim2000.RDS")
    
    tmp1 <- read.csv("result_df_collected_nsim2000_1-136.csv")
    tmp2 <- read.csv("result_df_collected_nsim2000_137+.csv")
    tmp <- rbind(tmp1, tmp2)
    write.csv(tmp, file = "result_df_collected_nsim2000.csv", row.names = FALSE)
}