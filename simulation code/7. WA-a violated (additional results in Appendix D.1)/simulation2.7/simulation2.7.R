...
# remove cases where f_w_shape is the same as f_t_shape
# SD <- SD[!(SD$ft_t_shape == SD$ft_w_shape), ]
...
for (i in 1:nrow(SD)) {
    ...
    dta <- dgm_noDE2(SD$n[i], m, ft_t, beta_t, gt_t, alpha_t, taut_t, pt)
}
...
if (0) {
    ...
    # result2 <- readRDS(paste0("../simulation2.7.1/result_df_collected_nsim", nsim, ".RDS"))
    ...
    result %>% 
        ...
        ggplot(aes(x = ft_w_theta, y = power_adj,
                   color = factor(ft_t_theta), linetype = ATE_ASPNC)) +
        # geom_point() +
        geom_line() + 
        ...
        facet_grid(gt_t_shape + gt_w_shape ~ ft_t_shape + ft_w_shape + gt_t_theta,
                   labeller = label_both) +
        theme_bw() + myggfont()
    ggsave(paste0("plot_simu2.7_power_ftwtheta.pdf"), width = 10, height = 7.5)
    
    result %>% 
        mutate(ATE_ASPNC = paste0(ATE_t, ", ", ASPNC_t)) %>%
        # pivot_longer(c("power_adj", "power_unadj"),
        #              names_to = "ss_correction", values_to = "power") %>%
        ggplot(aes(x = ft_t_theta, y = power_adj,
                   color = factor(ft_w_theta), linetype = ATE_ASPNC)) +
        # geom_point() +
        geom_line() + 
        geom_hline(yintercept = 0.8) +
        facet_grid(gt_t_shape + gt_w_shape ~ ft_t_shape + ft_w_shape + gt_t_theta,
                   labeller = label_both) +
        theme_bw() + myggfont()
    ggsave(paste0("plot_simu2.7_power_ftttheta.pdf"), width = 10, height = 7.5)
    
    result %>%
        mutate(ATE_ASPNC = paste0(ATE_t, ", ", ASPNC_t),
               ft_t_theta_minus_ft_w_theta = ft_t_theta - ft_w_theta) %>%
        ggplot(aes(x = ft_t_theta_minus_ft_w_theta, y = power_adj,
                   color = factor(ft_w_theta), linetype = ATE_ASPNC)) +
        # geom_point() +
        geom_line() + 
        geom_hline(yintercept = 0.8) +
        facet_grid(gt_t_shape + gt_w_shape ~ ft_t_shape + ft_w_shape + gt_t_theta,
                   labeller = label_both) +
        theme_bw() + myggfont()
    ggsave(paste0("plot_simu2.7_power_ftttheta_minus_ftwtheta.pdf"), width = 10, height = 7.5)
    
    result %>% 
        mutate(ATE_ASPNC = paste0(ATE_t, ", ", ASPNC_t)) %>%
        # pivot_longer(c("typeierror_adj", "typeierror_unadj"),
        #              names_to = "ss_correction", values_to = "typeierror") %>%
        ggplot(aes(x = ft_w_theta, y = typeierror_adj,
                   color = factor(ft_t_theta), linetype = ATE_ASPNC)) +
        # geom_point() +
        geom_line() + 
        ...
        facet_grid(gt_t_shape + gt_w_shape ~ ft_t_shape + ft_w_shape + gt_t_theta,
                   labeller = label_both) +
        theme_bw() + myggfont()
    ggsave(paste0("plot_simu2.7_typeierror_ftwtheta.pdf"), width = 10, height = 7.5)
    
    result %>% 
        mutate(ATE_ASPNC = paste0(ATE_t, ", ", ASPNC_t)) %>%
        # pivot_longer(c("typeierror_adj", "typeierror_unadj"),
        #              names_to = "ss_correction", values_to = "typeierror") %>%
        ggplot(aes(x = ft_t_theta, y = typeierror_adj,
                   color = factor(ft_w_theta), linetype = ATE_ASPNC)) +
        # geom_point() +
        geom_line() + 
        geom_hline(yintercept = 0.05) +
        facet_grid(gt_t_shape + gt_w_shape ~ ft_t_shape + ft_w_shape + gt_t_theta,
                   labeller = label_both) +
        theme_bw() + myggfont()
    ggsave(paste0("plot_simu2.7_typeierror_ftttheta.pdf"), width = 10, height = 7.5)
}
