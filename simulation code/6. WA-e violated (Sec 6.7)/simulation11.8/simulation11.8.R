...
SD <- expand.grid(gamma = 0.05,  # type I error
                  ...
                  # ft_t_shape = c("constant", "linear", "quadratic"),
                  gamma3 = seq(from = -0.2, by = 0.01, to = 0.2), # coefficent in dgm
                  gamma4 = seq(from = -0.2, by = 0.01, to = 0.2), # coefficent in dgm
                  # gt_t_shape = c("constant", "linear", "quadratic"),
...
SD <- SD[!(SD$ft_t_shape == "quadratic" & SD$gt_t_shape == "constant"),]
SD <- SD[!(SD$ft_t_shape == "quadratic" & SD$gt_t_shape == "linear"),]
SD <- SD[!(SD$ft_t_shape == "linear" & SD$gt_t_shape == "constant"),]
...
for (i in 1:nrow(SD)) {
    ...
    #                        SD$gt_t_MIF[i], coef_SC, pt)
    SD[i, c("alpha_t0", "alpha_t1", "alpha_t2")[1:SD$q[i]]] <- alpha_t
    # SD$ASPNC_t_empirical[i] <- compute_ASPNC(gt_t, alpha_t, taut_t, coef_SC, pt)
    ...
    ## taut_t
    taut_t <- compute_tau_endoAvail(gt_t, alpha_t, ft_t, beta_t, pt, gamma3, gamma4)
    SD$AvgTau_t[i] <- mean(taut_t)
    
    ## taut_w
    SD$AvgTau_w[i] <- SD$AvgTau_t[i]
    taut_w <- rep(SD$AvgTau_w[i], m)
    ...
    dta <- dgm_endoAvail(SD$n[i], m, ft_t, beta_t, gt_t, alpha_t, pt, gamma3, gamma4)
}
...
for (i in 1:nrow(SD)) {
    ...
    for (truth_hypothesis in c("H1", "H0")) {
        ...
        result <- foreach(isim = 1:nsim, .combine = "c", .options.snow = opts) %dorng% {
            library(rootSolve)
            dta <- dgm_endoAvail(n, m, ft_t, beta_t, gt_t, alpha_t, pt, gamma3, gamma4)
...
if (0) {
    ...
    result %>% 
        ...
        ggplot(aes(x = coef_SC, y = power_adj, color = ATE_ASPNC)) +
        geom_line() +
        geom_hline(yintercept = 0.8) +
        ggtitle(paste0("nsim = ", nsim)) +
        facet_grid(ft_t_shape ~ gt_t_shape, labeller = label_both) +
        theme_bw() + myggfont()
    ggsave(paste0("plot_simu7_power.pdf"), width = 10, height = 7.5)
    
    result %>% 
        # pivot_longer(c("power_adj", "power_unadj"),
        #              names_to = "ss_correction", values_to = "power") %>%
        mutate(ATE_ASPNC = paste0(ATE_t, ", ", ASPNC_t)) %>%
        ggplot(aes(x = coef_SC, y = typeierror_adj, color = ATE_ASPNC)) +
        geom_line() +
        geom_hline(yintercept = 0.05) +
        ggtitle(paste0("nsim = ", nsim)) +
        facet_grid(ft_t_shape ~ gt_t_shape, labeller = label_both) +
        theme_bw() + myggfont()
    ggsave(paste0("plot_simu7_typeierror.pdf"), width = 10, height = 7.5)
}
